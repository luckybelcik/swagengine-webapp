...
<div class="flex flex-row p-1 gap-1 items-center bg-base-200">
  <a
    href={page.url.pathname === '/editor' ? '/editor/settings' : '/editor'}
    aria-label="Open settings"
    tabindex="0"
    class="btn btn-square btn-lg bg-base-300 hover:bg-base-200"
  >
    <svg
      width="35px"
      height="35px"
  ...
  {#if isEditingName}
    <input
      type="text"
      class="input input-ghost text-xl"
  ...
  {:else}
    <button
        type="button"
        class="btn btn-ghost bg-base-300 text-xl p-2"
